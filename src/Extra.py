...
# Procesar el texto transcrito para extraer parámetros
def procesar_texto(texto):
    try:
        # Extraer tasa de llegada (landa) y otros parámetros
        landa = re.search(r'razón de\s*(\d+)\s*pacientes por hora', texto, re.IGNORECASE)
        tiempo_llegada = re.search(r'tiempo promedio de llegada de\s*(\d+)\s*minutos', texto, re.IGNORECASE)
        tiempo_servicio_min = re.search(r'media de\s*(\d+)\s*minutos', texto, re.IGNORECASE)
        tasa_servicio = re.search(r'tasa de servicio de\s*(\d+)\s*pacientes por hora', texto, re.IGNORECASE)
        capacidad_cola = re.search(r'no puede acomodar más de\s*(\d+)', texto, re.IGNORECASE)
        distribucion_llegada = re.search(r'distribución de Poisson', texto, re.IGNORECASE)
        distribucion_servicio = re.search(r'exponencial', texto, re.IGNORECASE)
        disciplina_cola = re.search(r'disciplina de la cola\s*([^\.,;]+)', texto, re.IGNORECASE)
        num_servidores = re.search(r'médico de acuerdo', texto, re.IGNORECASE)

        # Convertir y calcular los valores
        landa_valor = int(landa.group(1)) if landa else (int(tiempo_llegada.group(1)) if tiempo_llegada else None)
        mu_valor = int(tasa_servicio.group(1)) if tasa_servicio else (int(tiempo_servicio_min.group(1)) if tiempo_servicio_min else None)
        capacidad_cola_valor = int(capacidad_cola.group(1)) if capacidad_cola else None
        capacidad_sistema_valor = capacidad_cola_valor + 1 if capacidad_cola_valor else None
        ...
        num_servidores_valor = 1 if num_servidores else None
        capacidad_poblacion_valor = float('inf')  # Infinito
        disciplina_cola_valor = disciplina_cola.group(1).strip() if disciplina_cola else ""

        return {
            "landa": (landa_valor, "pacientes/hora", "Tasa de llegada" if landa else "Tiempo promedio de llegada"),
            "mu": (mu_valor, "pacientes/minuto", "Tasa de servicio" if tasa_servicio else "Tiempo promedio de servicio"),
...
# Ruta de la imagen
ruta_imagen = input("Por favor para comenzar, ingresa la ruta de la iamgen : ")
...
# Paso 2: Extraer los parámetros del texto
parametros = procesar_texto(texto_transcrito)
...
# Paso 3: Asignar respuestas automáticas
...
bienvenida='''\n Hola\U0001F44B\n¡Bienvenido a la calculadora interactiva de modelos de colas del proceso nacimiento y muerte: M/M/1, M/M/s, M/M/1/K y M/M/s/K en Python!
 Estimado usuario, aquí no solo obtendrás resultados numéricos, sino que aprenderás un poco sobre los modelos de colas.
 A medida que avanzamos, te guiaré en el proceso de ingreso de datos y te proporcionaré los supuestos de cada modelo.
 Los resultados obtenidos serán dados en unidades de minutos para los tiempos, y clientes/minuto para las tasas.
¡Comencémos!'''
...
if disciplina not in [1, 2, 3, 4]:
    print(f"\nDisciplina de cola no válida o no proporcionada. Se asumirá el valor predeterminado: Disciplina {disciplina}.")
...
if disciplina == 1:
    ...
    print(f"\n Supuesto {'8' if s == 1 else '9'}. La disciplina de la cola es Disciplina no especificada (valor predeterminado).")
